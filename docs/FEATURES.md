...
- **관련 핵심 파일/함수**: `AuthController.java#githubCallback`, `GitHubOAuthService.java`
...
- **관련 핵심 파일/함수**: `TotpController.java`, `TotpService.java`
...
- **관련 핵심 파일/함수**:
  - `analyze.py` — API 엔드포인트 및 스트리밍 로직
  - `sast_node.py` — LLM 분석 실행
  - `planning_node.py` — 분석 단계 계획
  - `graph_builder.py`
...
- **관련 핵심 파일/함수**:
  - `sast_node.py#get_github_file_content`
  - `get_repo_contents.ts`
  - `list_directory.ts`
...
- **관련 핵심 파일/함수**:
  - `AnalysisController.java#streamSession`
...
- **관련 핵심 파일/함수**:
  - `ProviderKeyController.java`
  - `sast_node.py#provider 결정 블록`
...
- **관련 핵심 파일/함수**:
  - `patch_node.py#_report_session_token_usage`
  - `CreditService.java`
  - `UserController.java#getCredits`
- **의존성**: PostgreSQL
